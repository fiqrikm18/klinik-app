...
namespace Apotik
{
    ...
    public partial class Login : Window
    {
        ...
        private void onNext(MonitorEvent ev)
        {
            try
            {
                if (ev.ToString() == "PCSC.Reactive.Events.CardInserted")
                {
                    ...
                    {
                        MessageBox.Show("Staff apoteker tidak terdaftar, hubungi administrator untuk mendaftar.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
            catch (Exception)
        ...
        {
            byte[] user = sp.ReadBlock(0x00, BlockId);
            Properties.Settings.Default.KodeApoteker = Utils.Util.ToASCII(user, 0, user.Length, false);
